...
function renderCancellationLine(){
  ...
  chartCancel = new Chart(ctx, {
    ...
    options: { responsive:true, plugins:{ tooltip:{ enabled:true, callbacks:{ label: ctx => `Rate: ${ (ctx.raw === null ? 'NaN' : (ctx.raw*100).toFixed(2)+'%') }` } } } }
  });
}
...
function renderPriceHistogram(){
  ...
  chartPrice = new Chart(ctx, { type:'bar', data:{ labels, datasets:[{ label:'Count', data: counts, backgroundColor:'#0288d1' }] }, options:{ responsive:true, plugins:{ tooltip:{ enabled:true } } }});
}
...
function renderCorrelationHeatmap(){
  ...
  chartHeat = new Chart(ctx, {
    ...
    options: {
      ...
      plugins:{ tooltip:{ enabled:true, callbacks:{ label: ctx => `${vars[ctx.raw.x]} vs ${vars[ctx.raw.y]}: ${ctx.raw.v.toFixed(3)}` } } }
    }
  });
}
...
btnPredict.addEventListener('click', async ()=>{
  ...
  // determine requested target: if user provided Year/Month within data, find index; else use last available and compute next month logically
  const targetIndex = series.findIndex(s => s.year===year && s.month===month);

  let seqWindow;
  if (targetIndex >= 12){
    // user asked for a month that exists and has at least 12 previous months
    seqWindow = series.slice(targetIndex-12, targetIndex);
  } else {
    // fallback: use last 12 months in series (train on end of available history)
    seqWindow = series.slice(-12);
    // and compute predicted month as next after last data row
