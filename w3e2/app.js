...
async function onEvaluate() {
  try {
    if (!model || !testXs || !testYs) {
      alert("Please train or load a model first.");
      return;
    }

    document.getElementById('data-status').textContent = "⏳ Evaluating on test set...";

    // Predict labels
    const preds = model.predict(testXs).argMax(-1); // [N]
    const labels = testYs.argMax(-1);               // [N]
    ...
    // === Overall accuracy ===
    const acc = tfvis.metrics.accuracy(yTrue, yPred);
    document.getElementById('metrics').textContent =
      `📊 Overall Test Accuracy: ${(acc * 100).toFixed(2)}%`;

    // === Confusion matrix ===
    const confusion = tfvis.metrics.confusionMatrix(yTrue, yPred);
    ...
    // === Per-class accuracy ===
    const perClass = tfvis.metrics.perClassAccuracy(yTrue, yPred);
    const perClassAcc = perClass.map((acc, i) => ({
      index: i,
      accuracy: acc
    }));
    ...
    document.getElementById('data-status').textContent = "✅ Evaluation complete!";
  } catch (err) {
    console.error(err);
    document.getElementById('data-status').textContent =
      "❌ Evaluation failed. See console for details.";
  }
}
