...
function predictTest() {
  ...
  if (!preprocessedTest.features || preprocessedTest.features.length === 0) { alert('Preprocessed test empty'); return; }
...
function downloadPredictions(format = "csv") {
  if (!processedTest || !processedTest.length) {
    alert("No predictions to download");
    return;
  }

  const xs = processedTest.map(r=>[
    r.Pclass, r.Sex, r.Age, r.SibSp, r.Parch, r.Fare, r.Embarked, r.IsAlone
  ]);
  const xsTensor = tf.tensor2d(xs);
  const probs = model.predict(xsTensor).arraySync().map(r=>r[0]);
  const preds = probs.map(p=>p>=threshold?1:0);
  const passengerIds = processedTest.map(r=>r.PassengerId);

  // Build rows
  const rows = passengerIds.map((id,i)=>({
    PassengerId: id,
    Survived: preds[i],
  ...
  if (format === "csv") {
    ...
    downloadBlob(csv, "submission.csv", "text/csv;charset=utf-8;");
    document.getElementById("predictionLog").innerText = "✅ CSV downloaded";
  } else if (format === "xlsx") {
    const ws = XLSX.utils.json_to_sheet(rows);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "Predictions");
    XLSX.writeFile(wb, "submission.xlsx");
    document.getElementById("predictionLog").innerText = "✅ Excel downloaded";
  }
}

function downloadBlob(text, filename, contentType) {
...
window.addEventListener('load', () => {
  ...
  const ids = ['run-eda-btn','export-merged-btn','preprocess-btn','create-model-btn','train-btn','stop-btn','eval-btn','predict-btn','download-btn','threshold-slider'];
