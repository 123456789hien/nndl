...
async function loadCSVFile(file){
  ...
  for(let i=0;i<lines.length;i++){
    ...
    if(parts.length<785){console.warn(`Skipping line ${i} cols=${parts.length}`); continue;}
...
function addNoise(xs,noiseStd=0.25){
  return tf.tidy(()=>{const noise=tf.randomNormal(xs.shape,0,noiseStd,'float32'); const noisy=xs.add(noise).clipByValue(0,1); return noisy.clone();});
}
...
function draw28x28ToCanvas(tensor,canvas,scale=4){
  ...
  const w=28,h=28; canvas.width=w*scale; canvas.height=h*scale;
