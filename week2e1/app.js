...
function exportPredictionsCSV() {
  ...
  alert('Predictions CSV downloaded.');
}

// ---------------------- Export Predictions + Model ----------------------
window.exportPredictionsWithModel = function() {
  if (!preprocessedTest || !preprocessedTest.predictions) { 
    alert('No predictions to export'); 
    return; 
  }
...
window.addEventListener('load', () => {
  ...
  const ids = ['run-eda-btn','export-merged-btn','preprocess-btn','create-model-btn','train-btn','stop-btn','eval-btn','conf-btn','predict-btn','download-btn','download1-btn','threshold-slider'];
