...
async function onLoadData() {
  ...
  try {
    ...
    const {trainXs, trainYs, valXs, valYs} = splitTrainVal(trainData.xs, trainData.ys, 0.1);

    window.trainXs = trainXs;
    window.trainYs = trainYs;
    window.valXs = valXs;
    window.valYs = valYs;
    window.testXs = testData.xs;
    window.testYs = testData.ys;

    document.getElementById('status').textContent = 
      `✅ Data loaded successfully: ${trainXs.shape[0]} train, ${valXs.shape[0]} val, ${testData.xs.shape[0]} test`;
  } catch (err) {
    console.error(err);
