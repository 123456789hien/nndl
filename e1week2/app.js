...
function loadData() {
  ...
  readerTrain.onload = e => {
    ...
    showPreview(trainData, "dataPreview", "Train Data (first 5 rows)", true);
    drawCharts(trainData);
  };
  ...
  readerTest.onload = e => {
    ...
    showPreview(testData, "dataPreview", "Test Data (first 5 rows)", false);
  };
...
// Helpers
// =========================
function showPreview(data, elementId, title, isTrain=true) {
  const preview = document.getElementById(elementId);
  preview.innerHTML += `<h3 class="font-bold text-pink-600">${title}</h3>`;

  let cols = Object.keys(data[0]);
  if (!isTrain) cols = cols.filter(c => c !== "Survived");
...
function cleanRow(row) {
  return {
    ...
    Survived: row.Survived !== undefined && row.Survived !== "" ? +row.Survived : null,
...
function preprocess() {
  ...
  function impute(data) {
    ...
    data.forEach(d => { if (!d.Embarked) d.Embarked = modeEmbarked; });
  ...
  impute(trainData);
  if (testData.length > 0) impute(testData);

  document.getElementById("preprocessInfo").innerText = "✅ Preprocessing done!";
}

// Encode features into numeric tensors
function encodeFeatures(data) {
  return data.map(r => [
    ...
    r.Fare ?? 0,
    r.Embarked === "C" ? 1 : 0,
    r.Embarked === "Q" ? 2 : (r.Embarked === "S" ? 3 : 0),
    r.FamilySize ?? 0,
    r.IsAlone ?? 0
  ]);
}
...
function drawCharts(data) {
  ...
  const sexValues = Object.keys(bySex).map(s => ({
    x: s,
    y: bySex[s].survived / bySex[s].total
  }));
  tfvis.render.barchart({ name: "Survival Rate by Sex", tab: "Charts" }, sexValues);
  ...
  const classValues = Object.keys(byClass).map(c => ({
    x: "Class " + c,
    y: byClass[c].survived / byClass[c].total
  }));
  tfvis.render.barchart({ name: "Survival Rate by Pclass", tab: "Charts" }, classValues);
}
...
function predictTest() {
  ...
  showPreview(predictions, "predictionPreview", "Prediction Results (first 5 rows)", false);
  xs.dispose();
}
