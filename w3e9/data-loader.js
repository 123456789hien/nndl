...
function addNoise(xs, noiseStd=0.25) {
  return tf.tidy(() => {
    ...
    return xs.add(noise).clipByValue(0,1).clone();
  });
}
...
function draw28x28ToCanvas(tensor, canvas, scale=4){
  let t = tensor;
  if (tensor.rank===4) t=t.reshape([28,28]);
  else if(tensor.rank===3 && tensor.shape[2]===1) t=t.reshape([28,28]);
  ...
  for(let i=0;i<data.length;i++){
    const v=Math.round(Math.max(0,Math.min(255,data[i]*255)));
