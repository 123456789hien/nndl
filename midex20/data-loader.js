...
function normalizeRow(row){
  ...
  // parse year/month robustly even if strings contain spaces or BOM
...
function aggregateMonthly(dataset){
  ...
  dataset.forEach(r => {
    if (!r.group) return;
    // if year/month missing, use 0 (these records won't affect normal months)
    const yr = r.year ?? 0;
    const mo = r.month ?? 0;
    ...
    for (let y=minY; y<=maxY; y++){
      for (let m=1; m<=12; m++){
        ...
        if (byKey.has(k)){
          ...
          // create zero rows for truly missing months
