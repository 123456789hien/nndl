...
let monthly = []; // aggregated monthly rows
...
btnLoad.addEventListener('click', async ()=>{
  ...
  try {
    ...
    noteLoad.innerText = `Loaded & aggregated: ${monthly.length} monthly rows across ${groups.length} groups`;
    renderTables();
    renderEDACharts();
...
function renderTables(){
  ...
  html += '</tbody></table>';
  mergeDiv.innerHTML = '<h3>Merge & Overview (preview)</h3>' + html;

  // --- NEW SECTION 1: Raw Data Preview (10 rows from each dataset) ---
  const rawPreview = [...rawTrain.slice(0,10), ...rawTest.slice(0,10)];
  let rawHtml = '<h3>Raw Data Preview (10 rows from each dataset)</h3><table><thead><tr>';
  if (rawPreview.length > 0) {
    Object.keys(rawPreview[0]).forEach(k => { rawHtml += `<th>${k}</th>`; });
    rawHtml += '</tr></thead><tbody>';
    rawPreview.forEach(r => {
      rawHtml += '<tr>' + Object.values(r).map(v => `<td>${v ?? ''}</td>`).join('') + '</tr>';
    });
    rawHtml += '</tbody></table>';
  } else {
  ...
  // --- NEW SECTION 2: Mean per month per room type (aggregated overview) ---
...
function populateGroupDropdown(){
  ...
  // default predict inputs to last available year/month
...
function renderCancellationLine(){
  ...
  chartCancel = new Chart(ctx, {
    type: 'line',
    data: { labels: labels.map(i=>i.toString()), datasets: [{ label: 'Avg Cancellation Rate (monthly)', data, borderColor:'#01497c', tension:0.2, pointRadius:3, spanGaps:true }] },
    options: { responsive:true, plugins:{ tooltip:{ callbacks:{ label: ctx => `Rate: ${ (ctx.raw === null ? 'NaN' : (ctx.raw*100).toFixed(2)+'%') }` } } } }
  });
}
...
function renderPriceHistogram(){
  ...
  const prices = monthly.map(d=>d.avg_room_price).filter(v=>v!==null && v!==undefined && !Number.isNaN(v));
  // create bins
  const bins = 20;
  ...
  chartPrice = new Chart(ctx, { type:'bar', data:{ labels, datasets:[{ label:'Count', data: counts, backgroundColor:'#0288d1' }] }, options:{ responsive:true }});
}
...
function renderCorrelationHeatmap(){
  ...
  // compute corr matrix
  const series = vars.map(v => monthly.map(d => (d[v]===null ? 0 : d[v])));
  function corr(a,b){
    const n = a.length;
  ...
  chartHeat = new Chart(ctx, {
    ...
    options: {
      ...
      plugins:{ tooltip:{ callbacks:{ label: ctx => `${vars[ctx.raw.x]} vs ${vars[ctx.raw.y]}: ${ctx.raw.v.toFixed(3)}` } } }
    }
  });
}
...
smoothingSlider.addEventListener('input', () => {
  ...
  // For simplicity, re-render charts using a smoothed copy of monthly for the cancellation series
  ...
  // temp substitute cancellation rates used by renderCancellationLine
  // We'll create a mapping of month_index -> avg(cancellation_rate_smoothed)
  const byIndex = new Map();
  smoothed.forEach(r=>{
...
function computeScaler(featuresArray){
  // featuresArray: array of [cancellation_rate, avg_room_price, lead_time_avg]
...
function scaleSample(sample){
  // sample: [cancellation_rate, avg_room_price, lead_time_avg]
...
btnTrain.addEventListener('click', async ()=>{
  // prepare sequences per group: for each group, take time-ordered rows, build sliding windows of 12 months
  const seqLen = 12;
  const groupsLocal = [...new Set(monthly.map(d=>d.group))];
  const X = []; const Y = [];
  groupsLocal.forEach(g=>{
    const seq = monthly.filter(x=>x.group===g).sort((a,b)=>a.month_index - b.month_index);
    // ensure values filled (we can forwardFill or linear interpolate before training; here we use forwardFill)
    // forward fill per group:
    let lastCancel = 0, lastPrice = 0, lastLead = 0;
    seq.forEach(s=>{
      if (s.cancellation_rate === null) s.cancellation_rate = lastCancel;
      else lastCancel = s.cancellation_rate;
      if (s.avg_room_price === null) s.avg_room_price = lastPrice;
      else lastPrice = s.avg_room_price;
      if (s.lead_time_avg === null) s.lead_time_avg = lastLead;
      else lastLead = s.lead_time_avg;
    });
  ...
  if (X.length === 0){ alert('Not enough data to build sequences (need at least 12 months per group).'); return; }
...
async function loadModelFromFiles(jsonFile, binFile){
  ...
  // tf.loadLayersModel supports browserFiles
...
btnPredict.addEventListener('click', async ()=>{
  ...
  const month = Number(inpMonth.value);
  if (!group){ alert('Choose a group'); return; }

  // pick series for group
  const series = monthly.filter(d=>d.group===group).sort((a,b)=>a.month_index-b.month_index);
  if (series.length < 12){ alert('Not enough history for this group'); return; }

  // find last 12 months ending at requested year-month (if that combination not present, use latest 12 available)
  const targetIndex = series.findIndex(s => s.year===year && s.month===month);
  let seqWindow;
  if (targetIndex >= 12){
    seqWindow = series.slice(targetIndex-12, targetIndex);
  } else {
    // fallback: use last 12 months in series
    seqWindow = series.slice(-12);
  }

  // apply interpolation strategy if chosen
  const interp = interpMethod.value;
  if (interp === 'ffill'){
    // forward fill fields in seqWindow
    ...
    // a quick linear interpolate across the 12-window for each field
    ['cancellation_rate','avg_room_price','lead_time_avg'].forEach(field=>{
      ...
      if (left === null){
        ...
        // forward/backwards
        // fill left side nulls with first known
    ...
    // none: replace null by 0 for prediction numeric stability
    seqWindow = seqWindow.map(s => ({
      ...s,
  ...
  // build input features [12,3]
  const features = seqWindow.map((s, idx) => [ smoothedCanc[idx], s.avg_room_price ?? 0, s.lead_time_avg ?? 0 ]);

  // scale features with scaler computed during training
  // If model was loaded (no training in this session), we require scaler to be present in local JS scope (we did compute during training)
  // Here we assume scaler is available (if user trained model in this session)
  if (!scaler.min || scaler.min.length === 0){
    // compute simple scaler from entire monthly dataset as fallback
    const allFeats = [];
    monthly.forEach(r => allFeats.push([ r.cancellation_rate ?? 0, r.avg_room_price ?? 0, r.lead_time_avg ?? 0 ]));
    computeScaler(allFeats);
  }
  const scaled = features.map(samp => scaleSample(samp)); // [12,3]

  // build tensor shape [1,12,3]
  const xt = tf.tensor3d([scaled]);

  // predict
  const ypred = await usedModel.predict(xt).array();
  const predVal = ypred[0][0]; // in original cancellation_rate scale (0..1)
  predictResult.innerText = `Predicted cancellation rate next month: ${(predVal*100).toFixed(2)}%`;
  ...
  // update predict chart (historic + predicted as red dot)
  ...
  const predIndex = xLabels.length; // next point
  const labels = [...xLabels, 'pred'];
  ...
  chartPredict = new Chart(ctx, {
    ...
    data: {
      labels,
      datasets: [
        { label: `${group} cancellation (smoothed)`, data: dataHist, borderColor:'#01497c', fill:false },
      ]
    },
