...
btnTrain.addEventListener('click', async ()=>{
  ...
  let rmseChart = null; // global variable to store Chart.js instance

function evaluateRMSE(predictions, actuals) {
  const diffs = predictions.map((p, i) => Math.pow(p - actuals[i], 2));
  const mse = diffs.reduce((a, b) => a + b, 0) / diffs.length;
...
async function trainModel() {
  ...
  // giả định bạn có x_train, y_train, x_test, y_test
  const epochs = parseInt(document.getElementById('epochs').value) || 80;
  const batchSize = parseInt(document.getElementById('batch-size').value) || 32;
  
  const model = buildModel(); // (giữ nguyên phần bạn có sẵn)
  ...
  for (let e = 1; e <= epochs; e++) {
    ...
    const rmse_train = evaluateRMSE(await y_pred_train.data(), await y_train.data());
    const rmse_test = evaluateRMSE(await y_pred_test.data(), await y_test.data());
    
    trainRMSEs.push(rmse_train);
    testRMSEs.push(rmse_test);
    
    document.getElementById("train-rmse").textContent =
      `Train RMSE (epoch ${e}): ${rmse_train.toFixed(5)}`;
    document.getElementById("test-rmse").textContent =
      `Test RMSE (epoch ${e}): ${rmse_test.toFixed(5)}`;
    
    // cập nhật biểu đồ theo từng epoch
    updateRMSEChart(trainRMSEs, testRMSEs);
  }

  console.log(`Training done. Final Train RMSE = ${trainRMSEs.at(-1).toFixed(5)}, Test RMSE = ${testRMSEs.at(-1).toFixed(5)}`);
  alert("✅ Training complete! Model ready for download.");

  // lưu final value
  document.getElementById("train-rmse").textContent =
    `Final Train RMSE: ${trainRMSEs.at(-1).toFixed(5)}`;
  document.getElementById("test-rmse").textContent =
    `Final Test RMSE: ${testRMSEs.at(-1).toFixed(5)}`;
}

function updateRMSEChart(trainRMSEs, testRMSEs) {
  const ctx = document.getElementById('rmse-chart').getContext('2d');
  const labels = trainRMSEs.map((_, i) => i + 1);
  ...
  rmseChart = new Chart(ctx, {
    ...
    options: {
      responsive: true,
      plugins: {
        title: { display: true, text: 'RMSE per Epoch' },
        tooltip: {
          callbacks: {
            label: function(context) {
...
// After single prediction result:
document.getElementById('predict-result').innerHTML = 
  `Predicted cancellation rate for ${month}/${year}: ${(pred * 100).toFixed(2)}%`;

// --- NEW FEATURE: Batch predict all Room_Types ---
const groups = [...new Set(monthly.map(r => r.room_type))];
let tableHTML = `<table><thead><tr><th>Room Type</th><th>Predicted Month</th><th>Cancellation Rate</th><th>Confidence</th></tr></thead><tbody>`;

for (const g of groups) {
  const predVal = predictForGroup(g, year, month); // same logic as current predict
  let conf = 'Low';
  if (predVal >= 0.5) conf = 'High';
  else if (predVal >= 0.2) conf = 'Medium';
  tableHTML += `<tr><td>${g}</td><td>${month}/${year}</td><td>${(predVal*100).toFixed(2)}%</td><td>${conf}</td></tr>`;
}
