...
document.getElementById('load-data').onclick=async()=>{
  try{
    ...
    if(!trainFile||!testFile) return alert('Select CSVs');
...
document.getElementById('load-model').onclick=async()=>{
  ...
  try{
    ...
    if(jsonFile.name.includes('denoiser')) modelDenoiser=loaded;
    else modelCNN=loaded;
    modelInfo.innerText+=`\nLoaded: ${loaded.name || jsonFile.name}`;
  }catch(e){console.error(e); alert('Load model error: '+e.message);}
};

// Evaluate
document.getElementById('evaluate').onclick=async()=>{
  if(!modelCNN||!testXs||!testYs) return alert('Need CNN & test data');
  const preds=modelCNN.predict(testXs);
  const predLabels=preds.argMax(1).dataSync();
  const trueLabels=testYs.argMax(1).dataSync();

  // Overall accuracy
  let correct=0, perClassAcc=Array(10).fill(0), perClassCount=Array(10).fill(0);
  ...
  confMatrix.dispose(); preds.dispose();
};
...
document.getElementById('test-five').onclick=()=>{
  ...
  for(let i=0;i<batch.xs.shape[0];i++){
    ...
    div.appendChild(canvas); const span=document.createElement('span'); span.innerText=`Label: ${label}`; div.appendChild(span);
