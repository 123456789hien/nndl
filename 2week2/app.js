...
//    Toggle FamilySize and IsAlone
// ----------------------
function preprocess() {
  if (!merged) { alert('Run EDA first'); return; }

  // Separate train rows
  const trainRows = merged.filter(r => r.source === 'train').map(r => ({...r})); // shallow copy

  // Impute Age (median) and Fare median, Embarked mode using train only
  ...
  trainRows.forEach(r => {
    ...
    const pclass = r.Pclass ? Number(r.Pclass) : 0;
    const sex = r.Sex ? String(r.Sex).toLowerCase() : 'male';
    const embarked = r.Embarked || embarkedMode;

    // features: Age_std, Fare_std, SibSp, Parch, one-hot Pclass (3), one-hot Sex (2), one-hot Embarked (3)
    const feat = [];
    feat.push( (ageVal - meanAge) / stdAge );
    feat.push( (fareVal - meanFare) / stdFare );
    feat.push( toNum(r.SibSp) || 0 );
    feat.push( toNum(r.Parch) || 0 );
    feat.push(...oneHot(pclass, [1,2,3]));          // Pclass 1,2,3
    feat.push(...oneHot(sex, ['male','female']));  // sex male/female
    feat.push(...oneHot(embarked, ['C','Q','S'])); // Embarked categories

    // optional family features
    const addFamily = document.getElementById('family-toggle').checked;
    if (addFamily) {
    ...
    X.push(feat);
    // label: ensure numeric 0/1
    const lab = r[TARGET] !== null && r[TARGET] !== undefined ? Number(r[TARGET]) : 0;
    Y.push(lab);
  });

  // Precompute test features similarly (but we shouldn't use test labels)
  ...
  testRows.forEach(r => {
    ...
    feat.push( toNum(r.SibSp) || 0 );
    feat.push( toNum(r.Parch) || 0 );
    feat.push(...oneHot(pclass, [1,2,3]));
    feat.push(...oneHot(sex, ['male','female']));
    feat.push(...oneHot(embarked, ['C','Q','S']));
...
function mode(arr) { const freq = {}; arr.forEach(v=>freq[v]=(freq[v]||0)+1); return Object.keys(freq).reduce((a,b)=> freq[a]>freq[b]?a:b); }
