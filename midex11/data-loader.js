...
let rawTest = [];
let monthlyAgg = [];
let groupKey = 'room_type';

// safe trim
function safeTrim(v){ return (v===null||v===undefined)?null:String(v).trim(); }

// normalize row
function normalizeRow(row){
  const out = {};
  out.ID = safeTrim(row['ID']||row['Id']||row['id']);
  out.n_adults = row['n_adults']===''?null:Number(row['n_adults']);
  ...
  out.status = row['status']===''?null:Number(row['status']);
...
function aggregateMonthly(dataset){
  ...
  dataset.forEach(r=>{
    if(!r.group) return;
    const yr = r.year??0, mo = r.month??0;
    const key = `${r.group}||${yr}||${mo}`;
    if(!map.has(key)) map.set(key,{group:r.group,year:yr,month:mo,total_bookings:0,cancelled_bookings:0,avg_room_price_sum:0,lead_time_sum:0,count_price:0,count_lead:0});
  ...
  map.forEach(v=>{
    ...
    arr.push({group:v.group,year:v.year,month:v.month,total_bookings:v.total_bookings,cancelled_bookings:v.cancelled_bookings,cancellation_rate:cancel_rate,avg_room_price:avg_price,lead_time_avg:lead_avg});
  });
  ...
  arr.forEach(d=>{d.month_index=(d.year-minYear)*12+(d.month-1);});
  ...
  groups.forEach(g=>{
    ...
    for(let y=minY;y<=maxY;y++){
      for(let m=1;m<=12;m++){
        ...
        else out.push({group:g,year:y,month:m,total_bookings:0,cancelled_bookings:0,cancellation_rate:0,avg_room_price:0,lead_time_avg:0,month_index:(y-minY)*12+(m-1)});
      }
    }
  });
