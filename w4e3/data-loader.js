class DataLoader {
        ...
        this.symbols.forEach(symbol => {
            ...
            this.dates.forEach(date => {
                ...
                this.normalizedData[symbol][date] = {
                    Open: (point.Open - minMax[symbol].Open.min) / (minMax[symbol].Open.max - minMax[symbol].Open.min),
                    Close: (point.Close - minMax[symbol].Close.min) / (minMax[symbol].Close.max - minMax[symbol].Close.min)
                };
            });
        });
    ...
    createSequences(sequenceLength = 12, predictionHorizon = 3) {
        ...
        for (let i = sequenceLength; i < this.dates.length - predictionHorizon; i++) {
            ...
            if (validSequence) {
                const target = [];
                const baseClosePrices = this.symbols.map(symbol => this.stocksData[symbol][currentDate].Close);
