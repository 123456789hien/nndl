...
// data-loader.js
  export async function loadTrainFromFiles(file) {
    ...
    for (let line of lines) {
      ...
      labels.push(vals[0]);
      images.push(vals.slice(1).map(v => v / 255.0));  // normalize
    }
  ...
  export async function loadTestFromFiles(file) {
    return loadTrainFromFiles(file);
  }
  ...
  export function getRandomTestBatch(xs, ys, k = 5) {
    return tf.tidy(() => {
      ...
      const allIdx = tf.util.createShuffledIndices(total);
      const selected = Array.from(allIdx).slice(0, k);
  
      // Convert to Tensor for gather
      const idxTensor = tf.tensor1d(selected, 'int32');
  
      const batchXs = xs.gather(idxTensor);
      const batchYs = ys.gather(idxTensor);
  
      idxTensor.dispose(); // clean up
